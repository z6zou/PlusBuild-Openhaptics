...
namespace MfVideoCapture
{
  ...
  //----------------------------------------------------------------------------
  HRESULT LogVideoAreaNew(const PROPVARIANT&)
  {
    return S_OK;
  }
  ...
  HRESULT SpecialCaseAttributeValueNew(GUID guid, const PROPVARIANT& var, MediaType& out)
  {
    ...
    else if (guid == MF_MT_FRAME_RATE_RANGE_MAX)   // if "undeclared identifier" error occurs then install Windows Platform SDK 7.1 or later
    {
      UINT32 uHigh = 0, uLow = 0;
