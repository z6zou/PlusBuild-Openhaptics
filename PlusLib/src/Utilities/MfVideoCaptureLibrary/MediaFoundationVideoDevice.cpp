...
namespace MfVideoCapture
{
  ...
  long MediaFoundationVideoDevice::ResetDevice(IMFActivate* pActivate)
  {
    ...
    this->CurrentStreamFormats.clear();
  ...
  MediaType MediaFoundationVideoDevice::GetFormat(unsigned int streamIndex, unsigned int formatIndex) const
  {
    ...
    if (formatIndex >= this->CurrentStreamFormats[streamIndex].size())
    {
      return MediaType();
    }
    return this->CurrentStreamFormats[streamIndex][formatIndex];
  }
  ...
  MfVideoCapture::format_sizet MediaFoundationVideoDevice::GetNumberOfFormats(unsigned int streamIndex) const
  {
    ...
    return this->CurrentStreamFormats[streamIndex].size();
  }
  ...
  bool MediaFoundationVideoDevice::FindType(unsigned int& foundFormatIndex, unsigned int streamIndex, unsigned int w, unsigned int h, unsigned int frameRate, GUID subtype)
  {
    if (streamIndex >= this->CurrentStreamFormats.size())
    {
      LOG_ERROR("MediaFoundationVideoDevice::FindType failed: stream " << streamIndex << " is not available (number of available streams: " << this->CurrentStreamFormats.size() << ")");
      return false;
    }
    if (frameRate == 0)
    {
  ...
  bool MediaFoundationVideoDevice::SetupDevice(unsigned int streamIndex, unsigned int formatIndex)
  {
    ...
    this->Width = this->CurrentStreamFormats[streamIndex][formatIndex].width;
    this->Height = this->CurrentStreamFormats[streamIndex][formatIndex].height;
    this->FrameRate = this->CurrentStreamFormats[streamIndex][formatIndex].MF_MT_FRAME_RATE;
  ...
  long MediaFoundationVideoDevice::EnumerateCaptureFormats(IMFMediaSource* pSource)
  {
    ...
    for (DWORD streamIndex = 0; streamIndex < descriptorCount; streamIndex++)
    {
      ...
      this->CurrentStreamFormats.push_back(formats);
    }
