...
bool PlusBkProFocusCameraLinkReceiver::Prepare(int samples, int lines, int pitch)
{
  ...
  if (lines < 0)
  {
    LOG_ERROR("Invalid lines argument sent to Prepare. Cannot process negative values.");
    return false;
  }

  unsigned int maxNumberOfLines = static_cast<unsigned int>(lines);
  ...
  if (numberOfRfSamplesPerLine + 6 < 0)
  {
    LOG_ERROR("numberOfRfSamplesPerLine is negative. Cannot continue.");
    return false;
  }
  m_NumberOfRfSamplesPerLine = static_cast<unsigned int>(numberOfRfSamplesPerLine + 6);
...
bool PlusBkProFocusCameraLinkReceiver::DataAvailable(int lines, int pitch, void const* frameData)
{
  ...
  unsigned int numberOfSamplesInInput = (pitch - HEADER_SIZE_BYTES) / BYTES_PER_SAMPLE / m_Decimation;
  unsigned int numberOfSamplePairsInInput = numberOfSamplesInInput / 2;
  unsigned int numberOfSamplePairsInOutput = m_NumberOfRfSamplesPerLine / 2;
  unsigned int numberOfSamplePairsToCopy = 0;
  ...
  unsigned int numBmodeLines = 0; // number of bmode lines in this m_Frame
  for (int inputLineIndex = 0; inputLineIndex < lines; ++inputLineIndex)
  {
    ...
    // (now there might be junk pixels at the end of the line if the line length is different for different lines)
  ...
  switch (m_ImagingMode)
  {
    ...
    case vtkPlusBkProFocusCameraLinkVideoSource::RfMode:
    {
      if (m_CallbackVideoSource != NULL)
      {
        ...
        FrameSizeType frameSizeInPix = {m_NumberOfRfSamplesPerLine, numBmodeLines, 1}; // each I and Q value is a sample (there are numRfSamples/2 IQ pairs in one line)
