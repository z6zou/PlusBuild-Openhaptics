...
int main(int argc, char **argv)
{
  ...
  vtkSmartPointer<vtkInfraredSeekCam> infraredSeekCam = vtkSmartPointer<vtkInfraredSeekCam>::New();

    infraredSeekCam->CreateDefaultOutputChannel();
...
// Add an observer to warning and error events for redirecting it to the stdout 
  ...
  infraredSeekCam->AddObserver("WarningEvent", callbackCommand); 
  infraredSeekCam->AddObserver("ErrorEvent", callbackCommand); 
  
  LOG_INFO("Initialize..."); 
  infraredSeekCam->Connect();

  if ( infraredSeekCam->GetConnected() )
  {
    LOG_INFO("Start recording..."); 
    infraredSeekCam->StartRecording(); 
  }
  else
  {
    infraredSeekCam->Disconnect();
    LOG_ERROR( "Unable to connect to IC capture device"); 
    exit(EXIT_FAILURE); 
  }

  LOG_INFO("Stop recording...");
  infraredSeekCam->StopRecording(); 
  infraredSeekCam->Disconnect();
