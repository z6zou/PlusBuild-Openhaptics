...
int SimpleCompareVolumes( vtkImageData* testVol, vtkImageData* refVol, double simpleCompareMaxError )
{
  ...
  imageDiff->SetInputData( 0, testVol );
  imageDiff->SetInputData( 1, refVol );
...
int main( int argc, char** argv )
{
  ...
  if ( roiOriginV.size() == 3 && roiSizeV.size() == 3 )
  {
    ...
    clipSlicesAlpha->SetInputData( slicesAlpha );
  ...
  if ( !outputAbsoluteDiffFileName.empty() )
  {
    ...
    writerAbs->SetInputData( histogramGenerator->GetOutputAbsoluteDifferenceImage() );
  ...
  if ( !outputTrueDiffFileName.empty() )
  {
    ...
    writerTru->SetInputData( histogramGenerator->GetOutputTrueDifferenceImage() );
